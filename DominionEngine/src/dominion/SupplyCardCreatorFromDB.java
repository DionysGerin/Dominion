...
public class SupplyCardCreatorFromDB {
    ...
    public ArrayList<TreasureCard> returnTreasureCardSupply(String treasureCardName, int amount) throws SQLException {
        ...
        try {
            ...
            if (res.next()) {
                cost = res.getInt("cost"); //Je moet res.next()callen en checken of die true is om de eerste lijn te van de resultaatset vast te krijgen 
            }
        ...
        for (int i = 0; i < amount; i++) {
            supplyCard.add(new TreasureCard(treasureCardName, cost, value));
        }
        return supplyCard;
    }
    ...
    public ArrayList<VictoryCard> returnVictoryCardSupply(String victoryCardName, int amount) throws SQLException {
        ...
        for (int i = 0; i < amount; i++) {
            supplyCard.add(new VictoryCard(victoryCardName, cost, value));
        }
        return supplyCard;
    }
    ...
    public ArrayList<KingdomCard> returnKingdomCardSupply(String kingdomCardName, int amount) throws SQLException {
        ...
        try {
            ...
            res = statm.executeQuery(sql);
            while (res.next()) {
                abilities.add(res.getInt("abilitynr"));
            }
        ...
        for (int i = 0; i < amount; i++) {
            supplyCard.add(new KingdomCard(kingdomCardName, cost, abilities));
        }
